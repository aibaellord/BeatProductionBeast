...
class AudioProcessor:
    ...
    def _initialize_processor(self):
        ...
        self.logger.info(
            f"Initializing AudioProcessor with sample rate {self.sample_rate}Hz, "
    ...
    def generate_waveform(
        ...
        """
        self.logger.debug(
            f"Generating {waveform_type} waveform at {frequency}Hz for {duration}s"
        )
        ...
        if waveform_type == "sine":
            ...
            audio_data = (
                amplitude * (2 / np.pi) * np.arcsin(np.sin(2 * np.pi * frequency * t))
            )
    ...
    def apply_filter(
        self, audio_data: np.ndarray, filter_type: str, filter_params: Dict
    ) -> np.ndarray:
        """
    ...
    def mix_audio_streams(
        self, streams: List[np.ndarray], weights: Optional[List[float]] = None
    ) -> np.ndarray:
        """
        ...
        for i, stream in enumerate(streams):
            if stream.shape != base_shape:
                raise ValueError(
                    f"Stream {i} shape {stream.shape} doesn't match base shape {base_shape}"
                )
        ...
        if weights is None:
            ...
            raise ValueError(
                f"Number of weights ({len(weights)}) doesn't match number of streams ({len(streams)})"
            )
...
class AudioSegment:
    ...
    def __post_init__(self):
        ...
        if len(self.data.shape) > 2:
            raise ValueError(
                f"Audio data should be 1D or 2D, got shape {self.data.shape}"
            )
...
class AudioProcessor:
    ...
    def normalize(
        self, audio: AudioSegment, target_db: Decibels = -3.0
    ) -> AudioSegment:
        """
    ...
    def resample(self, audio: AudioSegment, target_rate: SampleRate) -> AudioSegment:
        ...
        with ProcessingContext("resample", self.use_gpu):
            ...
            if self.use_gpu and CUDA_AVAILABLE:
                ...
                resampled_data = cusignal.resample_poly(
                    data_gpu, up=target_rate, down=audio.sample_rate
                )
                resampled_data = self.to_cpu(resampled_data)
            else:
                # Use CPU resampling
                resampled_data = signal.resample_poly(
                    audio.data, up=target_rate, down=audio.sample_rate
                )
...
class WaveManipulator(AudioProcessor):
    ...
    def generate_sine_wave(
        self, frequency: Hertz, duration: Seconds, amplitude: float = 1.0
    ) -> AudioSegment:
        """
        ...
        with ProcessingContext("generate_sine_wave", self.use_gpu):
            ...
            return AudioSegment(
                data=data, sample_rate=self.sample_rate, channels=1, duration=duration
            )
