...
def parse_arguments(args: List[str]) -> argparse.Namespace:
    ...
    style_group.add_argument(
        "--bpm", type=int, default=90, help="Beats per minute (tempo)"
    )
    style_group.add_argument(
        "--variation",
        type=float,
    ...
    pattern_group.add_argument(
        "--pattern-file", type=str, help="Load a specific pattern from a file"
    )
    pattern_group.add_argument(
        "--pattern-complexity",
        type=float,
    ...
    pattern_group.add_argument(
        "--bars", type=int, default=4, help="Number of bars to generate"
    )
    pattern_group.add_argument(
        "--swing", type=float, default=0.0, help="Swing amount to apply (0.0-1.0)"
    )
    ...
    export_group.add_argument(
        "--sample-rate", type=int, default=44100, help="Sample rate for audio exports"
    )
    export_group.add_argument(
        "--bit-depth",
        type=int,
...
def validate_arguments(args: argparse.Namespace) -> None:
    ...
    if not 0 <= args.pattern_complexity <= 1:
        raise ValueError(
            f"Pattern complexity must be between 0 and 1, got {args.pattern_complexity}"
        )
...
def export_beat(
    beat: Any, output_path: str, export_format: str, args: argparse.Namespace
) -> None:
    """
    ...
    logger.info(
        f"Export settings: Sample rate={args.sample_rate}, Bit depth={args.bit_depth}"
    )
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    try:
        ...
        logger.info(
            f"Successfully generated and exported {args.style} beat to {args.output}"
        )
        return 0
