...
class UnifiedUI:
    ...
    def _setup_youtube_interface(self):
        ...
        ttk.Checkbutton(
            upload_frame,
            text="Auto-Upload to YouTube",
            variable=self.auto_upload_var
        ).pack(side=tk.LEFT)
        
        ttk.Button(
            upload_frame,
            text="Schedule Uploads",
            command=self._schedule_uploads
        ).pack(side=tk.RIGHT)
    ...
    def _setup_analytics_interface(self):
        ...
        analytics_frame = ttk.LabelFrame(self.analytics_tab, text="Performance Analytics")
    ...
    def _run_full_automation(self):
        ...
        try:
            ...
            if result.get('success'):
                self.status_var.set(f"Successfully created {len(result['variations'])} variations!")
